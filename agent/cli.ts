 ...
 *
 * Usage: `npx tsx agent/cli.ts <work|news> [--model <model>] [--resume-session <sessionId>]`
 *
...
export function parseArgs(argv: string[]) {
  ...
  // Always new session unless an explicit --resume-session <id> is provided
  const newSession = !resumeSessionId;
...
if (isDirectRun) {
  ...
  if (!type || !validTypes.includes(type)) {
    console.error(
      `Usage: npx tsx agent/cli.ts <${validTypes.join("|")}> [--model <model>] [--resume-session <sessionId>]`,
    );
    process.exit(1);
  }
      ...
      logger.info(
        {
          ...
          durationMs: b.metadata.runDurationMs,
        },
        "Briefing complete",
      ),
    )
    .catch((err) => {
      logger.error({ err }, "Briefing failed");
      process.exit(1);
    });
}
