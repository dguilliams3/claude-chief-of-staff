...
function makeRepo(): string {
  ...
  copyFileSync(
    SCRIPT_SOURCE,
    join(repoDir, "scripts/check-depersonalization.sh"),
  );
  ...
  execFileSync("git", ["config", "user.name", "tests"], {
    cwd: repoDir,
    stdio: "ignore",
  });
  return repoDir;
...
describe("check-depersonalization.sh", () => {
  ...
  it("exits 0 for whitelisted file path even when patterns appear", () => {
    const repoDir = makeRepo();
    // The guard script whitelist includes `scripts/check-depersonalization.sh`
    // itself because it legitimately contains the forbidden patterns in its
    // FORBIDDEN regex constant. Stage a modified copy there to exercise the
    // whitelist branch.
    stageFile(
      repoDir,
      "scripts/check-depersonalization.sh",
